...
#include "UnityShadowLibrary.hlsl"
...
// ---- Screen space direction light shadows helpers (any version)
        ...
        inline half unitySampleShadow (unityShadowCoord4 shadowCoord)
        {
...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        ...
        half destName = tex2D(_LightTexture0, dot(lightCoord, lightCoord).rr).r * shadow;
#endif
...
inline half UnitySpotAttenuate(unityShadowCoord3 LightCoord)
{
...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        ...
        half shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        half destName = (lightCoord.z > 0) * UnitySpotCookie(lightCoord) * UnitySpotAttenuate(lightCoord.xyz) * shadow;
#endif

#ifdef DIRECTIONAL
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) half destName = UNITY_SHADOW_ATTENUATION(input, worldPos);
#endif
...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        DECLARE_LIGHT_COORD(input, worldPos); \
        half shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        half destName = tex2D(_LightTextureB0, dot(lightCoord, lightCoord).rr).r * texCUBE(_LightTexture0, lightCoord).w * shadow;
#endif
...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        DECLARE_LIGHT_COORD(input, worldPos); \
        half shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        half destName = tex2D(_LightTexture0, lightCoord).w * shadow;
#endif
